...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if ((lpMsg->CtlCode & 0x10) == 0x10)
		...
		//LogAddC(2, lMsg.Get(MSGGET(1, 245)), lpObj->AccountID, lpObj->Name);
		cManager.ManagerAdd(lpObj->Name, lpObj->m_Index);
		sprintf(szTemp, "[GM]%s is now online!", lpObj->Name);
		AllSendServerMsg(szTemp);
		}
	else if ((lpMsg->CtlCode & 0x20) == 0x20)
	{
	...
	if (lpObj->Authority == 16 || lpObj->Authority == 0x10) //Season3 add-on
	{
		gObjAddBuffEffect(lpObj, AT_GAMEMASTER_LOGO, 0, 0, 0, 0, -10);
	}
...
BOOL gObjSetPosMonster(int aIndex, int PosTableNum)
{
	...
	else if ( gMSetBase.GetPosition(PosTableNum, lpObj->MapNumber, lpObj->X, lpObj->Y) == FALSE )
	{
		...
		LogAdd("Monster location error-L2 : X : %d Y : %d map : %d", lpObj->X, lpObj->Y, lpObj->MapNumber);
		return FALSE;
	}
...
{
	if (aIndex <0 || aIndex > OBJMAX-1) ///possivel fix original: (OBJMAX - 1)
	{
		return 0;
	}
...
char* gObjGetAccountId(int aIndex)
{
	...
	if ( gObj[aIndex].Connected == 2 || g_AutoReconnect == 1)
	{
...
BOOL retResistance(LPOBJ lpObj, int Resistance_Type)
{
	...
	if ((lpObj->Authority & 16) == 16) //Season 2.5 add-on (GM Ring +255 Resistance Attribute)
	{
		if (lpObj->pInventory[10].m_Type == ITEMGET(13, 42) || lpObj->pInventory[11].m_Type == ITEMGET(13, 42))
		{
			return TRUE;
...
#ifdef __NOVUS__
	if (lpTargetObj->m_MUHelperOn)//FIX if( gMUHelper.GetUserSlot(lpTargetObj->m_Index) != (int)-1 )
	{
		...
		if(lpObj->Type == OBJ_USER)
		{
			...
			if(g_isCustomVipSys == 1)
			{
				...
				case 0:
					...
					break;
				case 1:
					exp = exp + (exp * g_vip1AddExp/100);
					break;
				case 2:
					exp = exp + (exp * g_vip2AddExp/100);
					break;
				case 3:
					exp = exp + (exp * g_vip3AddExp/100);
					break;
				case 4:
					exp = exp + (exp * g_vip4AddExp/100);
					break;
				case 5:
					exp = exp + (exp * g_vip5AddExp/100);
					break;
				case 6:
					exp = exp + (exp * g_vip6AddExp/100);
					break;
				default:
					exp = exp;
					break;
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, WORD Skill, int iShieldDamage)
{
	...
	if(lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
		...
		if(lpTargetObj->Type == OBJ_MONSTER)
		{
...
#endif
			if (lpTargetObj->Class == 161) // -> Kundun
			{
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "illusion of  The kundun has been killed by %s", lpObj->Name);
				AllSendServerMsg(Text);
			}
			
			if (lpTargetObj->Class == 181) // -> Kundun
			{
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "illusion of  The kundun has been killed by %s", lpObj->Name);
			...
			if (lpTargetObj->Class == 189) // -> Kundun
			{
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "illusion of  The kundun has been killed by %s", lpObj->Name);
			...
			if (lpTargetObj->Class == 197) // -> Kundun
			{
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "illusion of  The kundun has been killed by %s", lpObj->Name);
			...
			if (lpTargetObj->Class == 267) // -> Kundun
			{
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "illusion of  The kundun has been killed by %s", lpObj->Name);
			...
			if (lpTargetObj->Class == 338) // -> Kundun
			{
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "illusion of kundun has been killed by %s", lpObj->Name);
				AllSendServerMsg(Text);
			}
			...
			if (lpTargetObj->Class == 275) // -> Kundun
			 {
				char Text[256];
				ZeroMemory(Text, sizeof(Text));
				sprintf(Text, "The relic of kundun has been killed by %s", lpObj->Name);
				AllSendServerMsg(Text);
			}
...
BYTE gObjInventoryTradeMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	if ((lpObj->Authority & 16) != 16) //Season 2.5 add-on
	{
		if (lpObj->pInventory[source].m_Type == ITEMGET(14, 52))
		{
			return -1;
			...
			if(lpObj->DieRegen == 2)
			{
				...
				if(State != 0)
				{
					...
					if(lpObj->Type == OBJ_USER)
					{
						...
						if (lpObj->Authority == 16) //season4 add-on
						{
							gObjAddBuffEffect(lpObj, AT_GAMEMASTER_LOGO, 0, 0, 0, 0, -10);
						}
		...
		if(lpObj->Connected == PLAYER_CONNECTED ||	lpObj->Connected == PLAYER_LOGGED || lpObj->Connected == PLAYER_PLAYING)
		{
			if(lpObj->Type == OBJ_USER)
			{
				if(lpObj->Connected >= PLAYER_LOGGED)
				{
					...
					{
						if ((lpObj->IsOffTrade) && (!lpObj->m_bPShopOpen))
						{
							gObjDel(lpObj->m_Index);
						}
						else
...
{
	if ( mIndex < 0 || mIndex > MAX_MAGIC ) //fix removi o MAX_MAGIC-1 (funcionou?)
	{
...
BOOL gObjItemRandomOption3Up(LPOBJ lpObj, int source, int target)
{
	...
	LogAddTD("Item AD Option result code [%d]", lpObj->pInventory[target].m_Option3);
