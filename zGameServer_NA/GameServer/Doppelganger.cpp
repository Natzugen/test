...
void CDoppelganger::ProcDoppelgangerState_Playing(int current_time)
{
    ...
    if( !this->ukn2d0 )
    {
		if (this->ice_walker_dead = 0)//if( this->killer_state_ == 0)
        {
			if (GetTickCount() - this->ice_walker_spawn_time >= 30000)//if ( GetTickCount() - this->ukn36c >= 30000 )
            {
                this->ukn2d0 = 1;
...
void CDoppelganger::ArrangeMonsterHerd()
{
    ...
    {
		if (GetTickCount() - this->ice_walker_spawn_time > 60000)//if( GetTickCount() - this->ukn36c > 60000 )
        {
