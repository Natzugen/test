...
void CallCheckSumFromClient(int aIndex)
{
	/*Send Config to Main*/
	MAIN_CHECKSUM pAnswer = { 0 };
	PHeadSetW((LPBYTE)&pAnswer, 0xFC, sizeof(MAIN_CHECKSUM));
	DataSend(aIndex, (LPBYTE)&pAnswer, sizeof(MAIN_CHECKSUM));
	/*End Send Config to Main*/
}
...
void ProtocolCore(BYTE protoNum, BYTE * aRecv, int aLen, int aIndex, BOOL Encrypt, int serial)
{
	...
	//Check GXProtect
	/*if(g_GXProtectConnection == FALSE)
	CloseClient(aIndex);*/
	//END

	if(bCanConnectMember == 1)
	{
		ConMember.CheckVipStatus(gObj[aIndex].AccountID, gObj[aIndex].m_Index);
		...
		case 0xF1:
			{
				...
				/*Send Config to Main*/
				MIN_LEVEL_MUHELPER pAnswer = { 0 };
				PHeadSetW((LPBYTE)&pAnswer, 0xFF, sizeof(MIN_LEVEL_MUHELPER));
				pAnswer.lvl = g_MuHelperMinLevel;
				DataSend(aIndex, (LPBYTE)&pAnswer, sizeof(MIN_LEVEL_MUHELPER));
				/*End Send Config to Main*/
			}
			break;
						...
						if(g_AutoReconnect == 1)
						{
							...
							if(g_AutoBotStart == 1)
							{
								if(!gObj[aIndex].m_MUHelperOn)
								{
									...
									pAnswer.Mode = 0;
									// ----
									gObj[aIndex].m_MUHelperOn		= true;
									gObj[aIndex].m_MUHelperReadyPay	= false;
									gObj[aIndex].m_MUHelperStage		= 1;
									gObj[aIndex].m_MUHelperTick		= GetTickCount();

									// ----
									DataSend(gObj[aIndex].m_Index, (LPBYTE)&pAnswer, pAnswer.h.size);
									LogAddTD("[MUHelper] [%s] [%s] has been started", gObj[aIndex].AccountID, gObj[aIndex].Name);
								}
							}
						}
					}
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex) 
{
	...
	case '!':	// Global Announcement
		if ( slen > 2 )
		{
			if( CheckAuthorityCondition(32, lpObj))
			{
...
void CGPCharacterCreate( PMSG_CHARCREATE * lpMsg, int aIndex) 
{
	...
	//	{
	//		LogAddC(2, "error-L1: Fighter Character create error [%s]", gObj[aIndex].AccountID);
	//		JGCharacterCreateFailSend(aIndex, lpMsg->Name);
	//		return;
	//	}
	//}
	...
	else
	{
		...
		__try
		{
			...
			if ( lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE )
			{
				...
				//#ifdef __ALIEN__
				if(!g_OfflineTrade.UpdateExchange(lpObj,&gObj[aSourceIndex],dwCost))
					return;
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex) //0045C690
{
	...
	if( citem->IsItem() )
	{
		...
		else if(citem->m_Type == ITEMGET(14, 151))
		{
			g_QuestExpProgMng.ItemUseQuest(aIndex); //<--- new quest call to lua function
			gObjInventoryItemSet(aIndex, pos, -1);
			gObj[aIndex].pInventory[pos].Clear();
			GCInventoryItemDeleteSend(aIndex, pos, 1);
		}
...
void GCMonkDarkSideTargetSelect(PMSG_MONK_DARKSIDE_RECV * lpMsg, int aIndex)
{
	...
	if(gObj[aIndex].m_btDarkSideTargetNum == 0)
		gObj[aIndex].m_btDarkSideTargetNum = 1;
	// ----
	//FIX DARKSIDE
