...
#ifdef NEWWINGS
	this->NEW_JEWEL_180 = ITEMGET(14, 180);
	this->NEW_JEWEL_181 = ITEMGET(14, 181);
	this->NEW_JEWEL_182 = ITEMGET(14, 182);
	this->NEW_JEWEL_183 = ITEMGET(14, 183);
	this->NEW_JEWEL_184 = ITEMGET(14, 184);
	this->RARE_ITEM_TICKET_12 = ITEMGET(14, 185);
...
#ifdef NEWWINGS
	else if(iItemCode == this->NEW_JEWEL_180 || iItemCode == this->NEW_JEWEL_181 || iItemCode == this->NEW_JEWEL_182 
		|| iItemCode == this->NEW_JEWEL_183 || iItemCode == this->NEW_JEWEL_184 || iItemCode == this->RARE_ITEM_TICKET_12)
	{
		return true;
	}
#endif
...
void CMixSystem::NewWingsMix_Part1(LPOBJ lpObj)
{
	...
	for( int n = 0; n < CHAOS_BOX_SIZE; n++ )
	{
		if( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if(lpObj->pChaosBox[n].m_Type == ITEMGET(12,3))
				is2ndWing++;
			else if(lpObj->pChaosBox[n].m_Type == ITEMGET(12,4))
				is2ndWing++;
			else if(lpObj->pChaosBox[n].m_Type == ITEMGET(12,5))
				is2ndWing++;
			else if(lpObj->pChaosBox[n].m_Type == ITEMGET(12,6))
				is2ndWing++;
			else if(lpObj->pChaosBox[n].m_Type == ITEMGET(12, 15))
				JewelChaos++;
			else if(lpObj->pChaosBox[n].m_Type == ITEMGET(14, 22))
	...
	// ----
	if( is2ndWing == 0 || JewelChaos == 0 || RareItem12 == 0 || JewelCreation == 0 || InvalidItemCount > 0 )
	{
		DataSend(lpObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		lpObj->ChaosLock = FALSE;
		// ----
	...
	if( (rand()%100) < lpObj->ChaosSuccessRate )
	{
		...
		int RandomType		= rand() % 3; // -> from 0 to 5
		int WingsType		= 0;
		// ----
		int	WingsLuck		= false;
...
void CMixSystem::FourthWingMixFail(LPOBJ lpObj)
{
	if(lpObj->pChaosBox == NULL)
	{
		return;
	...
	for ( int n=0;n<CHAOS_BOX_SIZE;n++) //loc2
	{
		if ( lpObj->pChaosBox[n].IsItem() == TRUE )
		{
			if(Check4thWingItem(lpObj->pChaosBox[n].m_Type) != FALSE)
			{
				ThirdWingMixFailItemPanalty(&lpObj->pChaosBox[n]);
			}
			else if(CheckLevelCondition(&lpObj->pChaosBox[n], 0,0,0,0,1,0) != FALSE)
...
void CMixSystem::FourthWingMixFailItemPanalty(CItem * lpItem)
{
	if(lpItem == NULL)
	{
		return;
