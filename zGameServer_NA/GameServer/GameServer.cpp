...
BOOL GameServerStart(void) 
{
	...
	g_ConnectMemberUpdate = GetPrivateProfileInt("GameServerInfo","ConnectMemberUpdate",0,SERVER_INFO_PATH);
	//VIP SYSTEM
	g_isUpdateVipStatus = GetPrivateProfileInt("GameServerInfo", "IsVipUpdate", 0, SERVER_INFO_PATH);
	...
	SetTimer(ghWnd, WM_VIPSTATUSUPDATE, 60 * 1000, NULL);

	SetTimer(ghWnd, WM_VIPSTATUSREMOVE, 10 * 1000, NULL);
	return TRUE;
}
...
//#endif
				...
				KillTimer(ghWnd, WM_VIPSTATUSUPDATE);
				SetTimer(ghWnd, WM_VIPSTATUSUPDATE, 60 * 1000, NULL);

				KillTimer(ghWnd, WM_VIPSTATUSREMOVE);
				SetTimer(ghWnd, WM_VIPSTATUSREMOVE, 10 * 1000, NULL);
#ifdef QUESTSYSTEM
				g_QuestSystem.Load();
#endif
	...
	case WM_TIMER:
		...
		case WM_CONNECTMEMBERUPDATE:
			...
			//VIP SYSTEM
		case WM_VIPSTATUSUPDATE:
			{
				if(g_isUpdateVipStatus)
					ConMember.UpdateVipStatus();
			}break;
		case WM_VIPSTATUSREMOVE:
			{
				ConMember.RemoveVipStatus();
			}break;
			//END
